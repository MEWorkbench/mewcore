...
import pt.uminho.ceb.biosystems.mew.mewcore.simulation.formulations.ROOM;

public class ROOMSimulationKO {
...
//		listKOs.add(envKo15);
	...
	@Test
	public void ROOMAWithKO() throws Exception
	{
		...
		EnvironmentalConditions envKo1 = new EnvironmentalConditions();
		envKo1.put("R_EX_o2_e", new ReactionConstraint(0,0));
		
		GeneticConditions geneCond = new GeneticConditions(new ReactionChangesList(Arrays.asList(new String[]{"R_EX_o2_e"})));
		
		
		SimulationSteadyStateControlCenter simulationControlCenter = new SimulationSteadyStateControlCenter(envKo1, null, model, SimulationProperties.ROOM);
		
//		try {
			simulationControlCenter.setSolver(SolverType.GLPK);
			simulationControlCenter.setMaximization(true);
			//simulationControlCenter.setFBAObj(objectiveFunction);
			
//			if(this.controlSource.equals(ReferenceFluxDistributionSource.REFERENCE_FLUX_DISTRIBUTION)){
//				simulationControlCenter.setWTReference(fluxMeasures.getFluxValueList());
//			}
			
			SteadyStateSimulationResult solution = simulationControlCenter.simulate();
			
			for (String string : model.getReactions().keySet()) {
				System.out.println(string + " : "+ solution.getFluxValues().getValue(string));
			}
			
			System.out.println(solution.getOFString() + " : " + solution.getOFvalue());
			
			System.out.println(solution.getMethod());
...
//		
//		
//		ROOM simulation = new ROOM(model);
//		
//		simulation.setSolverType(SolverType.CPLEX);
//		simulation.setEnvironmentalConditions(envKo1);
//		simulation.setGeneticConditions(geneCond);
//		
//		SteadyStateSimulationResult result = simulation.simulate();
//		for (String string : model.getReactions().keySet()) {
//			System.out.println(string + " : "+ result.getFluxValues().getValue(string));
//		}
//		
