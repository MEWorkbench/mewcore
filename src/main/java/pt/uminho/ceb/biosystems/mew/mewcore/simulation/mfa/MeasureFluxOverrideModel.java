/*
 * Copyright 2010
 * IBB-CEB - Institute for Biotechnology and Bioengineering - Centre of Biological Engineering
 * CCTC - Computer Science and Technology Center
 *
 * University of Minho 
 * 
 * This is free software: you can redistribute it and/or modify 
 * it under the terms of the GNU Public License as published by 
 * the Free Software Foundation, either version 3 of the License, or 
 * (at your option) any later version. 
 * 
 * This code is distributed in the hope that it will be useful, 
 * but WITHOUT ANY WARRANTY; without even the implied warranty of 
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the 
 * GNU Public License for more details. 
 * 
 * You should have received a copy of the GNU Public License 
 * along with this code. If not, see http://www.gnu.org/licenses/ 
 * 
 * Created inside the SysBioPseg Research Group (http://sysbio.di.uminho.pt)
 */
package pt.uminho.ceb.biosystems.mew.mewcore.simulation.mfa;

import pt.uminho.ceb.biosystems.mew.mewcore.model.components.ReactionConstraint;
import pt.uminho.ceb.biosystems.mew.mewcore.model.steadystatemodel.ISteadyStateModel;
import pt.uminho.ceb.biosystems.mew.mewcore.simulation.components.AbstractOverrideSteadyStateModel;
import pt.uminho.ceb.biosystems.mew.mewcore.simulation.components.FluxValueMap;

public class MeasureFluxOverrideModel extends AbstractOverrideSteadyStateModel {

	protected FluxValueMap fluxValueList;
 
    public MeasureFluxOverrideModel(ISteadyStateModel model, FluxValueMap fluxMeasureList) {
    	this.model = model;
    	this.fluxValueList = fluxMeasureList;
    }

    @Override
    public ReactionConstraint getReactionConstraint(String reactionId) {   

    	if (fluxValueList.containsKey(reactionId))
    	{
    		double value = fluxValueList.get(reactionId);
    		return new ReactionConstraint(value, value);
    	}
    	else return super.getReactionConstraint(reactionId);
    }

    @Override
    public ReactionConstraint getReactionConstraint(int reactionIndex)
    {
    	String reactionId = model.getReactionId(reactionIndex);
    	return getReactionConstraint(reactionId);
    } 


}
