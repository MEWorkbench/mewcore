...
public class JecoliGOUConverter<T extends JecoliGenericConfiguration> implements IJecoliOptimizationStrategyConverter<T, GOUSolution> {
	...
	public ISolutionFactory<?> createSolutionFactory(T configuration, ISteadyStateDecoder decoder, AbstractMultiobjectiveEvaluationFunction<?> evaluationFunction) {
		...
		if (exponentBase != null) {
			((GOUDecoder) decoder).setExponentBase(exponentBase);
		}
		
		if (ouRange != null) {
			nmin = ouRange.getA();
			nmax = ouRange.getB();
			((GOUDecoder) decoder).setSpecialIndex(0);
		}
		...
		if (negativeAllowed) {
			...
			for (int i = nmin; i <= nmax; i++) {
				double v = ((GOUDecoder) decoder).convertExpressionValue(i);
				range[count] = v;
				if (v != 0.0) {
					range[count + 1] = -v;
					count++;
			...
			((GOUDecoder) decoder).setFixedRange(range);
			nmin = 0;
			nmax = maxIndex;
		}
