...
import pt.uminho.ceb.biosystems.mew.mewcore.simulation.components.AbstractSimulationMethodsFactory;
import pt.uminho.ceb.biosystems.mew.mewcore.simulation.components.AbstractSimulationSteadyStateControlCenter;
...
import pt.uminho.ceb.biosystems.mew.mewcore.simulation.mfa2.methods.MFAMethodsFactory;
import pt.uminho.ceb.biosystems.mew.mewcore.simulation.mfa2.methods.algebra.MFADetermined;
import pt.uminho.ceb.biosystems.mew.mewcore.simulation.mfa2.methods.algebra.MFALeastSquares;
import pt.uminho.ceb.biosystems.mew.mewcore.simulation.mfa2.methods.algebra.MFAWeightedLeastSquares;
import pt.uminho.ceb.biosystems.mew.mewcore.simulation.mfa2.methods.linearprogramming.MFALP;
import pt.uminho.ceb.biosystems.mew.mewcore.simulation.mfa2.methods.linearprogramming.MFAParsimonious;
import pt.uminho.ceb.biosystems.mew.mewcore.simulation.mfa2.methods.linearprogramming.MFAQP;
import pt.uminho.ceb.biosystems.mew.mewcore.simulation.mfa2.methods.nullspace.MFANullSpace;
import pt.uminho.ceb.biosystems.mew.mewcore.simulation.mfa2.methods.robustness.MFARobustnessAnalysis;
import pt.uminho.ceb.biosystems.mew.mewcore.simulation.mfa2.methods.variability.MFAFluxVariabilityAnalysis;
import pt.uminho.ceb.biosystems.mew.mewcore.simulation.mfa2.methods.variability.MFATightBounds;
...
public class MFAControlCenter extends AbstractSimulationSteadyStateControlCenter implements Serializable{
	 ...
	 * @param approach The approach to be used to perform the MFA, i.e., if it will be used either the NullSpace, (Algebra / LP), or QP approach
	 * @param systemType The type of the system, i.e.e, if it is either undetermined or determined or overvetermined */
	public MFAControlCenter(EnvironmentalConditions environmentalConditions,
	...
	/** This method can be used if the chosen approach had been the Null Space one 
	...
	/** This method can be used if the chosen approach had been the Null Space one 
	 * @param alternativeFluxesSolution indicates if the alternative fluxes solution should be computed */
	...
	/** This method can be used if the chosen approach had been the Classic Algebra and the system is overdetermined 
	...
	/** This method can be used if the chosen approach had been the Classic Algebra and the selected fitting is the weighed least squares 
	 * @param alpha if the Weighted LSQ fitting has been selected */
	public void setAlpha(Double alpha){
		addProperty(MFAProperties.MFA_ALGEBRA_WLSQ_ALPHA, alpha);
	}
	
	/** This method can be used if the chosen approach had been the Robustness analysis
	 * @param selectedFluxes is the set of fluxes for which the initial values will be varied */
	...
	/** This method can be used if the chosen approach had been the Robustness analysis
	 * @param percentageInterval is the percentage interval with which the initial values will be varied */
	...
	public void setApproach(MFAApproaches approach){
		addProperty(MFAProperties.MFA_APPROACH, approach);
		methodType = approach.getPropertyDescriptor();
	}
